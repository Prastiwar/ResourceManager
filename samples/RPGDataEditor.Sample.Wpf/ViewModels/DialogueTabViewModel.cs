...
{
    public class DialogueTabViewModel : PresentableCategoryDataViewModel<Dialogue>
    {
        ...
        protected override Dialogue CreateResource(PresentableData model) => new Dialogue() {
            Id = model.Id,
            Title = model.Name,
        ...
        protected override PresentableData CreatePresentableData(string location)
        {
            ...
            LocationResourceDescriptor pathDescriptor = DataSource.DescriptorService.GetRequiredDescriptor<LocationResourceDescriptor>(typeof(Dialogue));
            ...
            presentable.Name = parameters.FirstOrDefault(x => string.Compare(x.Key, nameof(Dialogue.Title), true) == 0).Value?.ToString();
