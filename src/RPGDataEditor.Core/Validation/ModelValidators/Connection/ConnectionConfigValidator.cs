...
namespace RPGDataEditor.Core.Validation
{
    public class ConnectionConfigValidator : AbstractValidator<IConnectionConfig>
    {
        ...
        {
            RuleFor(x => x.Get(nameof(ConnectionSettings.Type))).NotNull().WithMessage($"{nameof(ConnectionSettings.Type)} must be provided").WithName(nameof(ConnectionSettings.Type));
            ...
            RuleFor(x => x.Get("FolderPath", "")).Must(x => Directory.Exists(x.ToString())).WithMessage("Folder doesn't exists")
                                       .When(x => IsType(x, "local")).WithName("FolderPath");
            ...
            RuleFor(x => x.Get("UserName", "")).NotEmpty().WithMessage(CustomMessages.Empty)
                                       .When(x => IsType(x, "ftp"));
            RuleFor(x => (x.Get("Password", new SecureString()) as SecureString).Length).GreaterThan(0).WithMessage(CustomMessages.Empty)
                                                                    .When(x => IsType(x, "ftp") && x.Get("Password", new SecureString()) is SecureString);
