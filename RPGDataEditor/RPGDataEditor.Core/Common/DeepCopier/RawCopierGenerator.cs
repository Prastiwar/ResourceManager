...
namespace DeepCopy
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, DeepCopyDelegate<object>> Copiers = new ConcurrentDictionary<Type, DeepCopyDelegate<object>>();
        private static readonly Func<Type, DeepCopyDelegate<object>> GenerateCopier = CreateCopier;
        ...
        public static object Copy(object original, Type toType, CopyContext context)
        {
            ...
            DeepCopyDelegate<object> result = Copiers.GetOrAdd(toType, GenerateCopier);
        ...
        private static DeepCopyDelegate<object> CreateCopier(Type type)
        {
            if (type.IsArray)
            {
            ...
            return dynamicMethod.CreateDelegate(typeof(DeepCopyDelegate<object>)) as DeepCopyDelegate<object>;
        }

        private static DeepCopyDelegate<object> CreateArrayCopier(Type type)
        {
            ...
            return (DeepCopyDelegate<object>)methodInfo.MakeGenericMethod(elementType).CreateDelegate(typeof(DeepCopyDelegate<object>));
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static DeepCopyDelegate<object> ThrowNotSupportedType(Type type) => throw new NotSupportedException($"Unable to copy object of type {type}.");
    }
}
