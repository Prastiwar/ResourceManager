...
namespace DeepCopy
{
    ...
    internal sealed class MethodInfos
    {
        ...
        public MethodInfos()
        {
            ...
            MethodInfo GetFuncCall<T>(Expression<Func<T>> expression) => (expression.Body as MethodCallExpression)?.Method ?? throw new ArgumentException("Expression type unsupported.");
        }
    }
}